...
f_robot = @(t,X)complete_reg(X,x_star,y_star,theta_star,k1,k2,k3);
...
for i = 1:length(X)-1
    ...
    Xdot = complete_reg([XReal(i,1:2) thetareal],x_star,y_star,theta_star,k1,k2,k3);
...
%%
function Xdot = complete_reg(X,x_star,y_star,theta_star,k1,k2,k3)
x = X(1);
y = X(2);
theta = X(3);
...
ex = x_star - x;
ey = y_star - y;
etheta = delta_angle(theta_star,theta);
