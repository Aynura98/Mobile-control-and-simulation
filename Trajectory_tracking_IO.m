...
for i = 1:length(X)-1
    ...
    f_robot = @(t,X)(robot_control_FL(X,x_star,y_star,xd_star,yd_star,k1,k2,b));
    [t,evolution] = ode45(f_robot,[0,Ts],XReal(i,: ));
   
    [X_measured(:,i)] = trilat3(S,yk(:,i));
    ...
    plot_sensors_data(XReal(i,:),yk(:,i),h);% beacons - dashed red lines  
...
plot(X_Kestimated(3,:)-X(3,1:end-1),'--g','LineWidth',1);
...
plot(X_Kpredredicted(3,:)-X(3,1:end-1),'--g','LineWidth',1);
...
plot(X_Kpredredicted(3,:)-X_Kestimated(3,:),'--g','LineWidth',1);
plot(zeros(size(X_Kpredredicted,2)),'-k');
legend('First component','Second component','Third component','Location','best')
hold off
...
figure('Name','EKF')
hold on
plot(X_measured(1,:),X_measured(2,:),'or','LineWidth',1);    
plot(X_Kestimated(1,:),X_Kestimated(2,:),'-b','LineWidth',2);    
plot(X(:,1),X(:,2),'--k','LineWidth',1);    
plot(O(:,1),O(:,2),'ok','LineWidth',2);    
legend('Measured','Estimated','Location','best');
grid on
%% Trilateration 3rd form
function[theta_hat] = trilat3(beacons,dhat)
    N = size(beacons,1);
    Phi = [];
    b = [];
    for i = 1:N
        Bi = beacons(i,:);
        hi2 = dhat(i)^2-norm(Bi)^2;
        for j = i+1:N
            Bj = beacons(j,:);
            hj2 = dhat(j)^2-norm(Bj)^2;
            Phi = [ Phi;
                   -2*(Bi(1)-Bj(1)) -2*(Bi(2)-Bj(2))];
            b = [b;
                 hi2-hj2];
        end
    end
...
%%
function Xdot = robot_control_FL(X,x_star,y_star,xd_star,yd_star,kx,ky,b)
